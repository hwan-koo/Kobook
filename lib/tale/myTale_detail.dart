...
import '../quizs/quiz_main.dart';
import 'new_tales.dart';

// ignore: must_be_immutable
class MyTaleDetail extends ConsumerStatefulWidget {
  ...
  @override
  MyTaleDetailState createState() => MyTaleDetailState();
}

class MyTaleDetailState extends ConsumerState<MyTaleDetail> {
  @override
  void initState() {
  ...
  _asyncMethod() async {
    ...
    if (mounted) {
      setState(() {
        ...
        isPublic = prefs.getBool("${widget.timeStamp}_isPublic") ?? true;
      });
    }
  }
            ...
            content: Column(
              ...
              children: [
                TextField(
                  ...
                  cursorHeight: 15,
                  maxLength: Localizations.localeOf(context).toString() == "ko"
                      ? 12
                      : 30,
  ...
  Future<void> editText() async {
    FirebaseAnalytics.instance.logEvent(name: "EditTaleTitle");
    ...
    try {
      ...
      var url = Uri.parse(editTitleURL);
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        print(resBody);
        ref.watch(taleProviderModel.notifier).fetchTale();
  ...
  _setPrivate() async {
    FirebaseAnalytics.instance.logEvent(name: "SetPrivateTale");
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isPublic = false;
    });
    ...
    try {
      ...
      var url = Uri.parse(taleVisibility);

      var body = {"time_stamp": widget.timeStamp, "check": "0"};
      var req = http.Request('POST', url);
      req.headers.addAll(headersList);
      req.body = json.encode(body);
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        print(resBody);
        await prefs.setBool("${widget.timeStamp}_isPublic", false);
        setState(() {});
      } else {
        print(res.reasonPhrase);
      }
  ...
  _setPublic() async {
    FirebaseAnalytics.instance.logEvent(name: "SetPublicTale");
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    setState(() {
      isPublic = true;
    });
    ...
    try {
      ...
      var body = {"time_stamp": widget.timeStamp, "check": "1"};
      var req = http.Request('POST', url);
      req.headers.addAll(headersList);
      req.body = json.encode(body);
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        print(resBody);
        await prefs.setBool("${widget.timeStamp}_isPublic", true);
        setState(() {});
      } else {
        print(res.reasonPhrase);
      }
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    GestureDetector(
                      onTap: () {
                        // Airbridge.trackEvent("readMyTale");
                        FirebaseAnalytics.instance.logEvent(name: "ReadMyTale");
